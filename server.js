...
io.on('connection', function(socket){
    ...
    socket.on('newRocket', function(data) {
        //see the rocket when it is launched
        io.emit('newRocket',data);
        io.sockets.emit ('_rockets', data);
    });
    socket.on('newLaser', function(data) {
        io.emit('newLaser',data);
        io.sockets.emit ('_lasers', data);
    });
    socket.on('distributeDamage', function(data) {
        //distribute damage to the player who has been hit
        for(var i=0;i<room.length;i++){
            if(room[i].id === data.id){
                room[i].life -= data.damage;
                if(room[i].life <= 0){
                    room.splice(i, 1);
                    io.emit('disconnectPlayer',room); // all include sender
                }
                else{
                    io.emit('updateLife',[room,getTopFive(room)]); // all include sender
                }
            }
        }
        io.sockets.emit ('sendD', data);
    });
    ...
    socket.on('updateData', function(arrayData) {
        ...
        for(var i=0;i<room.length;i++){  
            if(room[i].id === idPlayer){
                var datasend = {};
                if(data.position){
                    room[i].position.x = data.position['_x'];
                    room[i].position.y = data.position['_y'];
                    room[i].position.z = data.position['_z'];
                ...
                if(data.rotation){
                    ...
                    room[i].rotation.x = data.rotation['_x'];
                    room[i].rotation.y = data.rotation['_y'];
                    room[i].rotation.z = data.rotation['_z'];
